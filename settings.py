...
SPREEDLY_CONFIRM_EMAIL_SUBJECT = getattr(settings, 'SPREEDLY_CONFIRM_EMAIL_SUBJECT', 'Complete your registration')   #'complete your subscription to %s' % Site.objects.get(id=settings.SITE_ID).name)

# the template to use for the confirmation email
SPREEDLY_GIFT_EMAIL = getattr(settings, 'SPREEDLY_GIFT_EMAIL', 'gift_email.txt')

# the subject for the confirmation email
SPREEDLY_GIFT_EMAIL_SUBJECT = getattr(settings, 'SPREEDLY_GIFT_EMAIL_SUBJECT', 'Gift subscription') #'gift subscription to %s' % Site.objects.get(id=settings.SITE_ID).name)
...
SPREEDLY_SITE_URL = getattr(settings, 'SPREEDLY_SITE_URL', 'http://example.com/') #Site.objects.get(id=settings.SITE_ID))
